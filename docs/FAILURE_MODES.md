...
| 5 | Scope creep: edits unrelated files | Out-of-scope edits (vs `GOAL.md` `## Scope`) block completion and are fed back | `driver/integrity.ts` `outOfScopeEdits` |
...
| 12 | Crash mid-run → lost/corrupt state | Atomic per-turn persist; recovery reopens any task whose recorded tree no longer matches | `lib/paths.ts` `atomicWriteJson`, `state/recover.ts` |
