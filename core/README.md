...
- `postToolUse` — capture ground-truth tool output under `.cursor/goal/driver/evidence/` (full payload in `tool-outputs/`, indexed tail + artifact pointer in `tool-runs.jsonl`). The hook does not inject bulky output into the agent; the driver reveals it progressively when needed.
