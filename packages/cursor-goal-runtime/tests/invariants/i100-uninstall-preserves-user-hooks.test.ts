...
describe("I100 uninstall preserves user hooks", () => {
  ...
  it("removes goal hook entries without deleting other commands on the same event", async () => {
    fakeCursorHome = path.join(os.tmpdir(), `i100-cursor-${Date.now()}`);
    fakeHome = path.join(os.tmpdir(), `i100-home-${Date.now()}`);
